...
void StudentTest(){
    ...
    Matrix m2(2,3,1);
    assert(m2.num_rows() == 2 && m2.num_cols() == 3);

    m2.set(0,0,0);
    m2.set(1,1,0);
    m2.set(1,2,0);

    m2.transpose(); //test if my transpose is correct
    m2.multiply_by_coefficient(7); // checks if my coefficient works

    m1 = m2; // my equal operator
    m1.subtract(m2); // checks if subtract works
    double *x = m2.get_col(1);
    std::cout << "New Matrix after -" << std::endl;
    std::cout << m1 << m2;
    std::cout << "\nThis is column(1 index 1) ";
    std::cout << x[1] << "\n";
    m1.clear();
    m2.clear();
    delete [] x; // delete my pointer that getcol created
